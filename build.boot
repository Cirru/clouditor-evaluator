...
(set-env!
 :dependencies '[[org.clojure/clojurescript "1.9.89"      :scope "test"]
                 ...
                 [mvc-works/respo           "0.3.7"       :scope "test"]])
...
(task-options!
  pom {:project     'Cirru/clouditor-evaluator
       :version     +version+
       :description "Cirru Evaluator"
       :url         "https://github.com/Cirru/clouditor-evaluator"
       :scm         {:url "https://github.com/Cirru/clouditor-evaluator"}
...
(defn html-dsl [data fileset]
  (make-html
    (html {}
    (head {}
      (title (use-text "Clouditor Evaluator"))
...
(deftask dev []
  ...
  (comp
    ...
    (watch)
    (transform-cirru)
    (reload :on-jsload 'clouditor-evaluator.main/on-jsload)
    (cljs)
    (target)))
...
(deftask rsync []
  (with-pre-wrap fileset
    (sh "rsync" "-r" "target/" "tiye:repo/mvc-works/clouditor-evaluator" "--exclude" "main.out" "--delete")
    fileset))
...
(deftask watch-test []
  ...
  (comp
    (watch)
    (transform-cirru)
    (test :namespaces '#{clouditor-evaluator-test.core})))
